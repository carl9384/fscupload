...
def send_upload_email_task(email, message,job_id):
    ...
    return send_upload_email(email, message,job)

@task(name="send_processing_complete_email_task")
def send_processing_complete_email_task(email,message,job_id):
    job = Fscjob.objects.get(pk=job_id)
    logger.info("Sent processing complete email notification")
    return send_processing_complete_email(email,message,job)
